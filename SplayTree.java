...
public class SplayTree implements Dictionary {
  ...
  private void splayNode(BinaryTreeNode node) {
    ...
    BinaryTreeNode grad = node.parent.parent;
    if (node.parent == root) {
      zig(node);
    } else if (grad.leftChild != null && grad.leftChild.leftChild != null && node == grad.leftChild.leftChild) {
      zigZig(node);
      splayNode(node);
    } else if (grad.rightChild != null && grad.rightChild.rightChild != null && node == grad.rightChild.rightChild) {
      zigZig(node);
      splayNode(node);
    } else if (grad.leftChild != null && grad.leftChild.rightChild != null && node == grad.leftChild.rightChild) {
      zigZag(node);
      splayNode(node);
    } else if (grad.rightChild != null && grad.rightChild.leftChild != null && node == grad.rightChild.leftChild) {
      zigZag(node);
      splayNode(node);
    }
